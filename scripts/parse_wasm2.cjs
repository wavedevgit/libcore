    ...
    if (sectionId === 11) {
        ...
        for (let i = 0; i < numSegments && p < end; i++) {
            ...
            if (mode === 0) {
                ...
                console.log(
                    `Segment ${i}: unknown mode ${mode} at file offset ${segStart}`,
                );
            }
            ...
            if (
                i < 5 ||
                (offsetVal >= 1073700 && offsetVal < 1075000) ||
                dataSize > 100
            ) {
                console.log(
                    `Segment ${i}: mode=${mode}, memIdx=${memIdx}, offset=${offsetVal}, dataLen=${dataSize}`,
                );
            }
...
for (const { offset, len } of keyOffsets) {
    ...
    console.log(
        `Offset ${offset} (len ${len}): raw="${raw}" decoded="${decoded.join('')}"`,
    );
}
