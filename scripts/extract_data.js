...
const FILE_PATH = '/home/wavedev/libcore/data/libdiscore.c';
const XOR_KEY = 0x73;
...
function parseDataString(str) {
    ...
    while (i < str.length) {
        if (str[i] === '\\') {
            if (i + 1 < str.length && str[i + 1] === '\\') {
            ...
            } else if (i + 3 <= str.length && /^[0-9a-fA-F]{2}$/.test(str.substring(i + 1, i + 3))) {
                // \XX format (hex byte)
                ...
                switch (ch) {
                    ...
                    default: bytes.push(ch.charCodeAt(0));
                }
                i += 2;
            } else {
...
function parseFile(filePath) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const inlineMatch = line.match(/^data\s+(\w+)\(offset:\s*(\d+)\)\s*=\s*"(.*)";/);
        if (inlineMatch) {
            // Save previous entry if exists
            if (currentOffset !== null && currentBytes.length > 0) {
                dataEntries.push({ offset: currentOffset, bytes: [...currentBytes] });
...
function findLength(dataEntries, targetOffset) {
    ...
    for (let i = 0; i < dataEntries.length; i++) {
        ...
        if (targetOffset >= entry.offset && targetOffset < entry.offset + entry.bytes.length) {
            return entry.offset + entry.bytes.length - targetOffset;
        }
    }
    
    throw new Error(`Could not find data entry containing offset ${targetOffset}`);
}
...
function main() {
    ...
    for (let i = Math.max(0, dataEntries.length - 5); i < dataEntries.length; i++) {
        const e = dataEntries[i];
        console.log(`  offset ${e.offset}: ${e.bytes.length} bytes`);
    }
    console.log();
    ...
    for (let i = 0; i < dataEntries.length; i++) {
        const entry = dataEntries[i];
        if (1073764 >= entry.offset && 1073764 < entry.offset + entry.bytes.length) {
            console.log(`  Found! Entry at offset ${entry.offset}, length ${entry.bytes.length}`);
            console.log(`  Contains offsets ${entry.offset} to ${entry.offset + entry.bytes.length - 1}`);
            break;
        }
    }
    console.log();
    ...
    for (const { offset, length } of fixedOffsets) {
        try {
            ...
            console.log(`  XOR 0x${XOR_KEY.toString(16)} decoded: ${bytesToString(decoded)}`);
    ...
    const bitmaskOffsets = [
        1073872, 1073908, 1073968, 1074092, 1074172,
        1074224, 1074316, 1074348, 1074372, 1074396, 1074448
    ];
