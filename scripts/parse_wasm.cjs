  ...
  if (section.id === 'data') {
    for (const entry of section.entries) {
      if (entry.init && entry.offset) {
        ...
        console.log(`Data section at memory offset ${offsetVal}, ${bytes.length} bytes`);
      }
    }
  }
}
...
function readU32(off) {
  return (mem[off] || 0) | ((mem[off+1] || 0) << 8) | ((mem[off+2] || 0) << 16) | ((mem[off+3] || 0) << 24);
}
...
for (const { offset, len } of keyOffsets) {
  ...
  const decoded = hexPairs.map(b => {
    const d = b ^ 0x73;
    return (d >= 0x20 && d < 0x7f) ? String.fromCharCode(d) : `\\x${d.toString(16).padStart(2, '0')}`;
  }).join('');
  console.log(`Offset ${offset} (len ${len}): raw="${raw}" decoded="${decoded}"`);
}
