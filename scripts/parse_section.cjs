const fs = require('fs');
const c = fs.readFileSync('./data/libdiscore.c', 'utf8');

// Find the section starting at 1073472
const startIdx = c.indexOf(
    'data d_zlrHYj_attemptedtotakeowners(offset: 1073472)',
);
...
// Check offsets
const targetOffsets = [
    1073764, 1073776, 1073778, 1073782, 1073872, 1073908, 1073968, 1074092,
    1074172, 1074224, 1074316, 1074348, 1074372, 1074396, 1074448,
];
...
for (const off of targetOffsets) {
    ...
    if (idx < 0 || idx >= bytes.length) {
        console.log(
            `Offset ${off}: OUT OF RANGE (idx=${idx}, total=${bytes.length})`,
        );
        continue;
    }
